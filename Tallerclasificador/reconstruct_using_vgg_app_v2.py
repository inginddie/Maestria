        ...
        for li, l in enumerate(head_layers):
            ...
            try:
                if classname == 'GlobalAveragePooling2D':
                    layer_obj = tf.keras.layers.GlobalAveragePooling2D(name=conf.get('name'))
                elif classname == 'Dense':
                    units = conf.get('units')
                    activation = conf.get('activation', None)
                    name = conf.get('name')
                    layer_obj = tf.keras.layers.Dense(units=units, activation=activation, name=name)
                elif classname == 'Dropout':
                    rate = conf.get('rate', conf.get('rate', 0.5))
                    name = conf.get('name')
                    layer_obj = tf.keras.layers.Dropout(rate=rate, name=name)
                else:
                    try:
                        layer_obj = tf.keras.layers.deserialize({'class_name': classname, 'config': conf})
                    except Exception:
                        print('Clase no soportada explícitamente y deserialize falló:', classname)
                        traceback.print_exc()
                        sys.exit(1)
            except Exception:
                print('Error creando instancia de la capa', classname)
                traceback.print_exc()
                sys.exit(1)
            try:
                x = layer_obj(x)
            except Exception:
                print('Error aplicando la capa al tensor', classname)
                traceback.print_exc()
                sys.exit(1)
